...
// Format-specific clip optimizer for platform-tailored exports
// Feature: format-clip-optimizer
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'format-clip-optimizer', payload, result);
    res.json({ success: true, feature: 'format-clip-optimizer', ...result });
  } catch (err) {
    console.error('format-clip-optimizer analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Format-specific clip optimizer for platform-tailored exports. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'format-clip-optimizer', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'format-clip-optimizer', title: 'Format-specific clip optimizer for platform-tailored exports', project: 'AiSplitVideos' });
});

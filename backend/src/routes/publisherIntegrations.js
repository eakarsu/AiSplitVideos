...
// Publisher integrations with scheduled cross-posting to social platforms
// Feature: publisher-integrations
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'publisher-integrations', payload, result);
    res.json({ success: true, feature: 'publisher-integrations', ...result });
  } catch (err) {
    console.error('publisher-integrations analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Publisher integrations with scheduled cross-posting to social platforms. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'publisher-integrations', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'publisher-integrations', title: 'Publisher integrations with scheduled cross-posting to social platforms', project: 'AiSplitVideos' });
});

...
// Speaker diarization and sentiment tracking with emotional-arc visualization
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'speaker-diarization', payload, result);
    res.json({ success: true, feature: 'speaker-diarization', ...result });
  } catch (err) {
    console.error('speaker-diarization analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Speaker diarization and sentiment tracking with emotional-arc visualization. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'speaker-diarization', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'speaker-diarization', title: 'Speaker diarization and sentiment tracking with emotional-arc visualization', project: 'AiSplitVideos' });
});

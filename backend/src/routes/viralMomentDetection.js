...
// Viral moment detection identifying high-engagement segments by sentiment/pacing
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'viral-moment-detection', payload, result);
    res.json({ success: true, feature: 'viral-moment-detection', ...result });
  } catch (err) {
    console.error('viral-moment-detection analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Viral moment detection identifying high-engagement segments by sentiment/pacing. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'viral-moment-detection', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'viral-moment-detection', title: 'Viral moment detection identifying high-engagement segments by sentiment/pacing', project: 'AiSplitVideos' });
});

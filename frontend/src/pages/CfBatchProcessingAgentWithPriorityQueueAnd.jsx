...
export default function CfBatchProcessingAgentWithPriorityQueueAnd() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-batch-processing-agent-with-priority-queue-and-retry/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-batch-processing-agent-with-priority-queue-and-retry', project: 'AiSplitVideos' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-batch-processing-agent-with-priority-queue-and-retry/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Batch processing agent with priority queue and retry logic</h1>

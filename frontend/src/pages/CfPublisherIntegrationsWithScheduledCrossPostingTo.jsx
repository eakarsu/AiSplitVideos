...
// Feature: Publisher integrations with scheduled cross-posting to social platforms
...
export default function CfPublisherIntegrationsWithScheduledCrossPostingTo() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-publisher-integrations-with-scheduled-cross-posting-to-social-platforms/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-publisher-integrations-with-scheduled-cross-posting-to-social-platforms', project: 'AiSplitVideos' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-publisher-integrations-with-scheduled-cross-posting-to-social-platforms/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Publisher integrations with scheduled cross-posting to social platforms</h1>

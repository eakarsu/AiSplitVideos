...
// Feature: Speaker diarization and sentiment tracking with emotional-arc visualization
...
export default function CfSpeakerDiarizationAndSentimentTrackingWithEmotional() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-speaker-diarization-and-sentiment-tracking-with-emotional-arc-visualization/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-speaker-diarization-and-sentiment-tracking-with-emotional-arc-visualization', project: 'AiSplitVideos' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-speaker-diarization-and-sentiment-tracking-with-emotional-arc-visualization/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Speaker diarization and sentiment tracking with emotional-arc visualization</h1>

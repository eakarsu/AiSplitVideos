...
// Feature: Viral moment detection identifying high-engagement segments by sentiment/pacing
...
export default function CfViralMomentDetectionIdentifyingHighEngagementSegments() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-viral-moment-detection-identifying-high-engagement-segments-by-sentiment/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-viral-moment-detection-identifying-high-engagement-segments-by-sentiment', project: 'AiSplitVideos' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-viral-moment-detection-identifying-high-engagement-segments-by-sentiment/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Viral moment detection identifying high-engagement segments by sentiment/pacing</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AiSplitVideos</p>
      </div>

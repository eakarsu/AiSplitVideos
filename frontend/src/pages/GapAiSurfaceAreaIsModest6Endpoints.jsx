...
export default function GapAiSurfaceAreaIsModest6Endpoints() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-ai-surface-area-is-modest-6-endpoints-relative/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-ai-surface-area-is-modest-6-endpoints-relative', project: 'AiSplitVideos' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-ai-surface-area-is-modest-6-endpoints-relative/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">AI surface area is modest (6 endpoints) relative to UI features</h1>

...
export default function GapNoAiPoweredSubtitleCaptionOptimizationEndpoint() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-powered-subtitle-caption-optimization-endpoint/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-ai-powered-subtitle-caption-optimization-endpoint', project: 'AiSplitVideos' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-powered-subtitle-caption-optimization-endpoint/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No AI-powered subtitle/caption optimization endpoint</h1>

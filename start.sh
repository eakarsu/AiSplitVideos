...
start_services(){ check_config;[ -d "$API_DIR/node_modules" ]||fail "API dependencies are missing; install explicitly";[ -d "$UI_DIR/node_modules" ]||fail "UI dependencies are missing; install explicitly";(cd "$API_DIR" && node src/migrate.js && node src/provision-demo.js);(cd "$API_DIR" && PORT="$BACKEND_PORT" FRONTEND_PORT="$FRONTEND_PORT" node src/index.js) & api_pid=$!;(cd "$UI_DIR" && PORT="$FRONTEND_PORT" REACT_APP_BACKEND_PORT="$BACKEND_PORT" BROWSER=none npm start) & ui_pid=$!;trap 'kill "$api_pid" "$ui_pid" 2>/dev/null || true;wait "$api_pid" "$ui_pid" 2>/dev/null || true' INT TERM EXIT;wait "$api_pid" "$ui_pid"; }
